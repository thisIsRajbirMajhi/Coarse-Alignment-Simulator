...
from src.local_terminal.association import associate_acquisition, associate_tracking
from src.local_terminal.comm_receiver import CommReceiver, CommSource
from src.local_terminal.detector import TemporalConfirmer, detect_spots
from src.local_terminal.identity import IdentityValidator
from src.local_terminal.models import AutonomyConfig, BeaconObservation
from src.local_terminal.reacquisition import V2Reacquisition
from src.local_terminal.registry import SignatureRegistry
from src.local_terminal.scan import ScanController
from src.local_terminal.selector import select_active_v2
from src.local_terminal.tracker import KalmanConfig, KalmanTracker, r_for_snr
...
class SupervisorV2:
    ...
    def _init_ai(self) -> None:
        ...
        Spec simple reliable AI: when ai_simple_mode True, only verifier (single lightweight CNN)
        is loaded; scorer/predictor/ranker are disabled (classical fallback for those paths).
        Default ai_simple_mode False preserves backward compat (all 4 models if available).
        """
        if not bool(getattr(self.cfg, "ai_enabled", False)):
            return
        simple = bool(getattr(self.cfg, "ai_simple_mode", False))
        try:
        ...
        if simple:
            # Spec: single CNN only — disable other models explicitly
            self.ai_scorer = None
        ...
        try:
            ...
            # keep verifier if it succeeded; others None
            if self.ai_verifier is None:
                self.ai_verifier = self.ai_scorer = self.ai_predictor = self.ai_ranker = None
            else:
                self.ai_scorer = None
                self.ai_predictor = None
    ...
    def step(self, fov_frame: np.ndarray, dt: float, sim_time_s: float,
        ...
        if bool(getattr(self.cfg, "ai_enabled", False)) and self.ai_verifier is not None:
            try:
                ...
                from src.local_terminal.detector import detect_spots as _ds
