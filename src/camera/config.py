...
class CameraConfig(BaseValidatedConfig):
    ...
    # Simple mode: bypass physics (inertia/backlash/encoder) for pan+=clamp(rate)*dt
    ...
    def validate(self) -> "CameraConfig":
        ...
        self.simple_mode = bool(self.simple_mode)
        return self
...
class PIDConfig(BaseValidatedConfig):
    ...
    Tuned for 640x480 4x3deg FOV: kp 3.0 settles 120px error in ~0.4s (12 frames) vs 1.2s at 1.5.
    """
    # Pan axis gains
    kp_pan: float = 5.0
    ki_pan: float = 0.12
    ...
    tau: float = 0.015                    # Derivative filter time constant (seconds)
    ...
    max_output_deg_s: float = 8.0        # Velocity command limit (deg/s) — within spec 5-10
