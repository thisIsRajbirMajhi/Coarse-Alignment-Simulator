...
from src.camera.state import PTZState
...
class PTZCamera:
    ...
    def set_simple_mode(self, enabled: bool) -> None:
        """Toggle simple_mode at runtime (per spec pan+=clamp(rate)*dt)."""
        self.config.simple_mode = bool(enabled)
    ...
    def update(
        ...
        # Simple mode fast path: pan+=clamp(rate)*dt bypassing inertia/backlash/encoder
        if bool(getattr(self.config, "simple_mode", False)):
            pan_rate = float(np.clip(pan_cmd, -self.config.max_pan_speed_deg_s, self.config.max_pan_speed_deg_s))
            tilt_rate = float(np.clip(tilt_cmd, -self.config.max_tilt_speed_deg_s, self.config.max_tilt_speed_deg_s))
            self.pan_deg = float(np.clip(self.pan_deg + pan_rate * dt_eff, eff_pan_min, eff_pan_max))
            self.tilt_deg = float(np.clip(self.tilt_deg + tilt_rate * dt_eff, eff_tilt_min, eff_tilt_max))
            self.pan_vel_deg_s = float(pan_rate)
            self.tilt_vel_deg_s = float(tilt_rate)
            self.pan_accel_deg_s2 = 0.0
            self.tilt_accel_deg_s2 = 0.0
            # In simple mode measured == true (no quantization/noise)
            self._measured_pan_deg = float(self.pan_deg)
            self._measured_tilt_deg = float(self.tilt_deg)
    ...
    def _compute_measured_angles(self) -> tuple[float, float]:
        ...
        In simple_mode measured == true (bypass encoder).
        """
        if bool(getattr(self.config, "simple_mode", False)):
            self._measured_pan_deg = float(self.pan_deg)
            self._measured_tilt_deg = float(self.tilt_deg)
