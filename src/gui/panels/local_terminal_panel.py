...
class LocalTerminalPanel(BaseConfigPanel):
    ...
    def _build_ui(self) -> None:
        ...
        self.chk_ai_simple = QCheckBox("Simple (verifier-only)")
        self.chk_ai_simple.setStyleSheet("color:#374151; font-size:11px;")
        self.chk_ai_simple.setToolTip("Spec simple reliable AI: only Tiny-CNN 32×32 verifier; scorer/predictor/ranker disabled (classical fallback). OFF = full hybrid (4 models).")
        ...
        ai_grid.addWidget(self.chk_ai_simple, 1, 0, 1, 2)
        self.lbl_ai_status = QLabel("AI models: checking…")
        self.lbl_ai_status.setStyleSheet("color:#6b7280; font-size:10px; font-style:italic;")
        self.lbl_ai_status.setWordWrap(True)
        ai_grid.addWidget(self.lbl_ai_status, 1, 2, 1, 2)
        ai_grid.addWidget(self._hint("AI is advisory: classical FSM wins. FPS ≥28 ON, ≥35 OFF. No GPU. Threshold 0.6, fusion 0.35/0.10/0.15/0.20/0.20."), 2, 0, 1, 4)
    ...
    def _watched_widgets(self):
        base = [
            self.chk_ai, self.spin_ai_thr, self.chk_ai_simple,
    ...
    def collect_config(self) -> AutonomyConfig:
        ...
        cfg = AutonomyConfig(
            ...
            ai_simple_mode=bool(self.chk_ai_simple.isChecked()),
    ...
    def set_config(self, cfg, emit: bool = False) -> None:
        ...
        try:
            ...
            self.chk_ai_simple.setChecked(bool(getattr(cfg, "ai_simple_mode", False)))
            # Update AI model status badge (which ONNX present, device)
            try:
                import os as _os
                from pathlib import Path as _P
                has_v = _P("src/local_terminal/ai/models/verifier.onnx").exists() and _P("src/local_terminal/ai/models/verifier.onnx").stat().st_size > 1000
                has_s = _P("src/local_terminal/ai/models/scorer.onnx").exists() and _P("src/local_terminal/ai/models/scorer.onnx").stat().st_size > 500
                has_p = _P("src/local_terminal/ai/models/predictor.onnx").exists() and _P("src/local_terminal/ai/models/predictor.onnx").stat().st_size > 500
                has_r = _P("src/local_terminal/ai/models/ranker.onnx").exists() and _P("src/local_terminal/ai/models/ranker.onnx").stat().st_size > 500
                dev = "cpu"
                try:
                    import torch as _t
                    ...
                    pass
                if getattr(cfg, "ai_simple_mode", False):
                    status = f"Models ({dev}): verifier={'OK' if has_v else 'missing(heur)'} - simple mode (other models off)"
                else:
                    status = f"Models ({dev}): verifier={'OK' if has_v else 'missing'} scorer={'OK' if has_s else 'missing'} predictor={'OK' if has_p else 'missing'} ranker={'OK' if has_r else 'missing'}"
                self.lbl_ai_status.setText(status)
            except Exception:
                pass
