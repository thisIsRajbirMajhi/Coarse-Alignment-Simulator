...
class RunMode(str, Enum):
    """Execution mode for SimulationSession.

    SIMULATION: normal scene/render pipeline (default, backward-compat).
    BENCHMARK_VIDEO: frames from .mp4 via VideoSource, PTZ bypass.
    REPLAY: deterministic log replay (reserved, behaves like benchmark bypass).
    """
...
class SimulationSession:
    ...
    def __init__(self, env_config=None, disturbance_config=None, scenario_config=None,
        ...
        from src.remote_terminal import make_default_scenario
    ...
    def build(self) -> None:
        ...
        from src.remote_terminal import RemoteTerminalManager
    ...
    # -- RunMode / video source API (backward-compat) --
    @property
    ...
    def is_benchmark(self) -> bool:
        """True when PTZ/disturbance must be bypassed (video or replay)."""
    ...
    def set_video_source(self, path: str | Path | None, config: Any | None = None, ground_truth_csv: str | Path | None = None) -> None:
        """Attach a .mp4 benchmark source. Pass None/empty to clear.

        Args:
            path: Path to .mp4 video. None clears benchmark mode.
            config: Optional VideoSourceConfig.
            ground_truth_csv: Optional CSV with per-frame x,y.
        """
        ...
        try:
            ...
            log.info("SimulationSession benchmark video attached: %s", path)
        except Exception as e:
            log.warning("set_video_source failed for %s: %s", path, e)
            raise
    ...
    def set_run_mode(self, mode: RunMode | str) -> None:
        """Explicit mode switch; clears video source if switching to SIMULATION."""
    ...
    def _step_benchmark(self, dt_eff: float) -> FrameSnapshot:
        """Benchmark path: read video frame, bypass PTZ/disturbance, preserve timestamps."""
        pkt = None
        try:
            pkt = self._video_source.next()  # type: ignore[union-attr]
        except Exception as e:
            log.debug("benchmark video next() failed: %s", e)
            pkt = None
        if pkt is None:
            # EOS: return last cached frame or empty
            fov_frame = getattr(self, "_cached_world_frame", None)
            if fov_frame is None:
                import numpy as _np
                fov_frame = _np.zeros((480, 640, 3), dtype=_np.uint8)
            world_frame = fov_frame
            dcx, dcy = 320.0, 240.0
            self._frame_id += 1
            self._sim_time_s += dt_eff
            # Still tick supervisor with blank frame to keep telemetry flowing
            try:
                from src.local_terminal import CommSource as _CS
                sup_out = self.supervisor.step(
                    fov_frame=fov_frame, dt=dt_eff, sim_time_s=self._sim_time_s,
                    comm_sources=[], boresight_world=(float(dcx), float(dcy)),
                    cam_home=self.camera.get_home(),
                    px_per_deg=(self.camera.config.px_per_deg_h, self.camera.config.px_per_deg_v),
                    origin_shift=(0.0, 0.0), fov_size=(int(self.camera.fov_width), int(self.camera.fov_height)),
                )
            except Exception:
                sup_out = type("O", (), {"pid_active": False, "track_error_px": None, "camera_target_angles": None, "target_vel_px_s": None, "telemetry": {}, "camera_reset_requested": False})()  # type: ignore
            self._pending_pid_active = getattr(sup_out, "pid_active", False)
            self._pending_track_error = getattr(sup_out, "track_error_px", None)
            self._pending_target_angles = getattr(sup_out, "camera_target_angles", None)
            self._pending_target_vel = getattr(sup_out, "target_vel_px_s", None)
            try:
                cam_st = self.camera.get_state()
                pan, tilt = cam_st.pan_deg, cam_st.tilt_deg
                cam_tel = self.camera.get_telemetry()
                pid_tel = self.controller.get_telemetry()
            except Exception:
                pan = tilt = 0.0
                cam_tel = pid_tel = None
            return FrameSnapshot(
                ...
                fov_frame=fov_frame, camera_telemetry=cam_tel,
                pid_telemetry=pid_tel, dt=dt_eff,
                terminals=self._safe_remote_telemetry(), pan=pan, tilt=tilt,
                fov_size=(self.camera.fov_width, self.camera.fov_height),
                tracker_telemetry=getattr(sup_out, "telemetry", {}),
                fov_center_disturbed=(float(dcx), float(dcy)),
                fov_rect_disturbed=None,
            )
        # Normal benchmark frame
        image = pkt.image
        fov_frame = pkt.fov_frame if pkt.fov_frame is not None else image
        world_frame = image  # God view shows same as FOV in benchmark (no world)
        # Preserve timestamp from packet
        try:
            self._sim_time_s = float(pkt.timestamp_s)
        except Exception:
            self._sim_time_s += dt_eff
        # Synthesize boresight at image centre (PTZ bypass)
        h, w = fov_frame.shape[:2]
        dcx, dcy = float(w) / 2.0, float(h) / 2.0
        # Keep disturbed center in sync
        ...
        # Still update scene/remote time for telemetry consistency but skip rendering
        try:
        ...
        # If packet has ground_truth, inject a synthetic source for scoring
        if pkt.ground_truth is not None:
            try:
                ...
                # Map GT from image coords to world (benchmark has no world; use fov centre as proxy)
                sources.append(CommSource(
                    position=(gx, gy), emitting=True, power_w=1.0,
                    chip_at=None, pointing_error_deg=0.0, wavelength_nm=1550.0,
                    beam_diameter_m=0.1, range_m=1000.0,
                ))
            except Exception:
                pass
        # Also add live terminal sources if any
        ...
        sup_out = self.supervisor.step(
            ...
            cam_home=self.camera.get_home(),
            px_per_deg=(self.camera.config.px_per_deg_h, self.camera.config.px_per_deg_v),
            origin_shift=shift, fov_size=(int(w), int(h)),
        )
        ...
        self._pending_target_vel = getattr(sup_out, "target_vel_px_s", None)
        # PTZ bypass: do NOT call camera.update regardless of supervisor output
        # (spec requires PTZ bypass in benchmark mode)
        ...
        self._cached_world_frame = world_frame
        cam_st = self.camera.get_state()
        return FrameSnapshot(
            frame_id=self._frame_id, world_frame=world_frame,
            world_size=(int(self.env_config.world_width), int(self.env_config.world_height)),
            ...
            terminals=self._safe_remote_telemetry(), pan=cam_st.pan_deg, tilt=cam_st.tilt_deg,
            fov_size=(int(w), int(h)),
    ...
    def step(self, dt: float) -> FrameSnapshot:
        ...
        # PTZ bypass: if benchmark video is active, delegate
