...
from src.common.rng import substream
...
from src.remote_terminal.models import RemoteScenarioRuntime, Vector2
...
class RemoteTerminalManager:
    ...
    def update(self, dt: float) -> RemoteScenarioRuntime:
        ...
        # Single-beacon simple mode: bypass formation offsets, one beacon at center
        if self._single_beacon():
            # Keep single terminal at formation center
            runtimes = []
            if self.terminals:
                pos = Vector2(float(center.x), float(center.y))
                runtimes.append(
                    self.terminals[0].step(
                        dt=dt,
            ...
            # Still update remaining terminals off-screen at same center to keep generators ticking but not rendered
            for terminal in self.terminals[1:]:
                pos = Vector2(float(center.x), float(center.y))
                terminal.step(dt=dt, sim_time_s=self.sim_time_s, position_m=pos, velocity_mps=Vector2(velocity.x, velocity.y), reference_m=self.reference_m, rng=self._point_rng)
        else:
            runtimes = []
    ...
    def render_spots(self, frame: np.ndarray) -> np.ndarray:
        ...
        Simple mode: single 10px square at center bypassing formation offsets.
        Normal: 2D gaussian per terminal.
        """
        if frame is None or not self.terminals:
            return frame
        if self._single_beacon():
            return self._render_single_beacon_square(frame, world_space=True)
        out = frame
        h, w = frame.shape[:2]
    ...
    def render_spots_on_fov(self, fov_frame: np.ndarray, fov_rect: tuple[float, float, float, float]) -> np.ndarray:
        ...
        if fov_frame is None or not self.terminals:
            return fov_frame
        if self._single_beacon():
            return self._render_single_beacon_square(fov_frame, world_space=False, fov_rect=fov_rect)
        out = fov_frame
    ...
    def _render_single_beacon_square(self, frame: np.ndarray, world_space: bool = True, fov_rect: tuple[float,float,float,float] | None = None) -> np.ndarray:
        """Draw single 10px filled square at center (simple mode)."""
        if frame is None or not self.terminals:
            return frame
        term = self.terminals[0]
        if not term.runtime.effective_emission_enabled or float(term.runtime.instantaneous_power_w) <= 0:
            return frame
        h, w = frame.shape[:2]
        if world_space:
            cx = term.position_m.x * M_PER_PX
            cy = term.position_m.y * M_PER_PX
        else:
            assert fov_rect is not None
            x0, y0, _, _ = fov_rect
            cx = term.position_m.x * M_PER_PX - x0
            cy = term.position_m.y * M_PER_PX - y0
        half = 5  # 10px square
        x0i, y0i = int(round(cx - half)), int(round(cy - half))
        x1i, y1i = x0i + 10, y0i + 10
        # Clip to frame
        fx0, fx1 = max(0, x0i), min(w, x1i)
        fy0, fy1 = max(0, y0i), min(h, y1i)
        if fx1 <= fx0 or fy1 <= fy0:
            return frame
        peak = float(np.clip(float(term.runtime.instantaneous_power_w) / POWER_REF_W, 0.0, 1.0))
        # Filled square at ~peak brightness, 3-channel safe
        if frame.ndim == 3:
            frame[fy0:fy1, fx0:fx1] = np.clip(frame[fy0:fy1, fx0:fx1].astype(float) + peak*255, 0, 255).astype(frame.dtype)
        else:
            frame[fy0:fy1, fx0:fx1] = np.clip(frame[fy0:fy1, fx0:fx1].astype(float) + peak*255, 0, 255).astype(frame.dtype)
        return frame

    # -- internals ---------------------------------------------------
    def _single_beacon(self) -> bool:
        return bool(getattr(self.config, "single_beacon_mode", False))

    def set_single_beacon_mode(self, enabled: bool) -> None:
        self.config.single_beacon_mode = bool(enabled)
        self._offsets = self._compute_offsets()

    def _compute_offsets(self) -> list[Vector2]:
        if bool(getattr(self.config, "single_beacon_mode", False)):
