...
# Provides FSOCEnv with standard gym API: reset(seed) -> obs, step(action) -> obs,reward,terminated,truncated,info
# If gymnasium is not installed, falls back to lightweight GymLike base.
...
def _make_sim_from_config(headless_config: HeadlessConfig, seed: int) -> HeadlessSimulation:
    """Helper to map HeadlessConfig short names to HeadlessSimulation long names."""
    return HeadlessSimulation(
        ...
        detector_config=headless_config.detector,
...
if _HAS_GYM:
    class FSOCEnv(gym.Env):  # type: ignore
        """
        Gymnasium Env for FSOC — headless, deterministic, AI-ready.

        Observation: Dict {
            "image": Box(0,255,(H,W,3),uint8)  — FOV frame,
            "vector": Box(-inf,inf,(6,),float32) — [err_x, err_y, pan, tilt, vx_est, vy_est],
            "lock": Discrete(4) — 0=searching,1=acquired,2=tracking,3=lost
        }
        Action: Box(-clamp, clamp, (2,)) — (d_pan, d_tilt) px delta; None uses internal PID.

        Example:
            import gymnasium as gym
            env = FSOCEnv(seed=42)
            obs, info = env.reset(seed=42)
            obs, reward, terminated, truncated, info = env.step(np.array([2.0, -1.0]))
        """
        metadata = {"render_modes": ["rgb_array", "human"], "render_fps": 30}

        def __init__(self, seed: int = 42, headless_config: HeadlessConfig | None = None, render_mode: str | None = None, **kwargs):
            super().__init__()
            # Handle kwargs that may be full config names (env_config etc) -> map to HeadlessConfig short names
            # Filter kwargs to HeadlessConfig fields
            cfg_kwargs = {}
            for k, v in kwargs.items():
                if k in HeadlessConfig.__dataclass_fields__:
                    ...
                    cfg_kwargs["beacon"] = v
                elif k == "detector_config":
                    cfg_kwargs["detector"] = v
        ...
        def step(self, action):
            ...
            # Gym terminated vs truncated: terminated on task success/failure, truncated on time
        ...
        def _to_gym_obs(self, obs_dict: dict):
            ...
            # Vector: [err_x, err_y, pan, tilt, vx, vy]
    ...
    class FSOCEnv:  # type: ignore
        """Fallback FSOCEnv without gymnasium — same reset/step API, gym-like obs (image/vector/lock)."""
        ...
        def observation_space(self):
            h, w = int(self.sim.camera_config.fov_height), int(self.sim.camera_config.fov_width)
            # Dict-like for fallback (no spaces)
            return {"image": (h,w,3), "vector": (6,), "lock": 4}
