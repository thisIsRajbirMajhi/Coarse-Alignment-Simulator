# gui/multi_beacon_panel.py - Single unified beacon panel: all beacon/target parameters in one panel

from PyQt5.QtCore import pyqtSignal
from PyQt5.QtWidgets import (
    QCheckBox,
    QComboBox,
    QGridLayout,
    QGroupBox,
    QLabel,
    QPushButton,
    QSpinBox,
    QVBoxLayout,
    QWidget,
)

from target.config import MultiBeaconConfig
from target.constants import BEACON_SHAPES, MOTION_PROFILES_DISPLAY, MULTI_BEACON_LIMITS


class MultiBeaconPanel(QWidget):
    """
    Single panel for all beacon/target settings:
      - Beacon count (1-5) and Target index
      - Shape: Square / Circle / Random (per beacon random if Random)
      - Size: Width 5-20, Height 2-20, default 10x10
      - Initial location: X,Y user defined + Random button (random if multiple)
      - Motion: Straight Line, Circular, Figure 8, Spiral, Sin, Zig-Zag, Random
      - Speed + Random toggle
      - Blinking toggle
      - Randomize parameters (all) and Randomize motion
    All beacons share same rules; Random distributes per beacon.
    """

    multiConfigChanged = pyqtSignal(object)
    targetChanged = pyqtSignal(int)
    randomizeAllRequested = pyqtSignal()
    randomizeMotionRequested = pyqtSignal()
    threshChanged = pyqtSignal(int)

    def __init__(self, initial: MultiBeaconConfig | None = None, world_bounds: tuple[int, int] = (5000, 5000), parent=None):
        super().__init__(parent)
        self._world_bounds = world_bounds
        self._config = (initial or MultiBeaconConfig(beacon_count=1, target_index=0)).validate()
        self._build_ui()
        self.set_config(self._config, emit=False)

    def _build_ui(self) -> None:
        root = QVBoxLayout(self)
        root.setContentsMargins(0, 0, 0, 0)
        root.setSpacing(10)

        box = QGroupBox("Beacon / Target")
        grid = QGridLayout(box)
        grid.setContentsMargins(12, 18, 12, 12)
        grid.setHorizontalSpacing(8)
        grid.setVerticalSpacing(8)
        grid.setColumnStretch(1, 1)
        grid.setColumnStretch(3, 1)

        # Row 0: Count and Target
        grid.addWidget(self._label("Count"), 0, 0)
        self.spin_beacon_count = QSpinBox()
        lo, hi = MULTI_BEACON_LIMITS["beacon_count"]
        self.spin_beacon_count.setRange(int(lo), int(hi))
        self.spin_beacon_count.setToolTip("Number of targets (1-5)")
        self.spin_beacon_count.setMinimumHeight(26)
        grid.addWidget(self.spin_beacon_count, 0, 1)

        grid.addWidget(self._label("Target"), 0, 2)
        self.spin_target_index = QSpinBox()
        self.spin_target_index.setRange(0, 4)
        self.spin_target_index.setToolTip("Which beacon is target")
        self.spin_target_index.setMinimumHeight(26)
        grid.addWidget(self.spin_target_index, 0, 3)

        # Row 1: Shape
        grid.addWidget(self._label("Shape"), 1, 0)
        self.combo_shape = QComboBox()
        self.combo_shape.addItems([s.capitalize() for s in BEACON_SHAPES])
        self.combo_shape.setToolTip("Target shape: Square, Circle, Random (each beacon random)")
        self.combo_shape.setMinimumHeight(26)
        grid.addWidget(self.combo_shape, 1, 1)

        # Blinking toggle
        self.chk_blinking = QCheckBox("Blinking")
        self.chk_blinking.setToolTip("Toggle blinking for beacons/target")
        grid.addWidget(self.chk_blinking, 1, 2, 1, 2)

        # Row 2: Size W and H
        grid.addWidget(self._label("Size W"), 2, 0)
        self.spin_size_w = QSpinBox()
        self.spin_size_w.setRange(5, 20)
        self.spin_size_w.setValue(10)
        self.spin_size_w.setSuffix(" px")
        self.spin_size_w.setToolTip("Target width 5-20 px")
        self.spin_size_w.setMinimumHeight(26)
        grid.addWidget(self.spin_size_w, 2, 1)

        grid.addWidget(self._label("H"), 2, 2)
        self.spin_size_h = QSpinBox()
        self.spin_size_h.setRange(2, 20)
        self.spin_size_h.setValue(10)
        self.spin_size_h.setSuffix(" px")
        self.spin_size_h.setToolTip("Target height 2-20 px")
        self.spin_size_h.setMinimumHeight(26)
        grid.addWidget(self.spin_size_h, 2, 3)

        # Row 3: Initial location X,Y
        grid.addWidget(self._label("Init X"), 3, 0)
        self.spin_x = QSpinBox()
        self.spin_x.setRange(0, 5000)
        self.spin_x.setValue(2500)
        self.spin_x.setSuffix(" px")
        self.spin_x.setToolTip("Initial X — user defined, random default for multiple beacons")
        self.spin_x.setMinimumHeight(26)
        grid.addWidget(self.spin_x, 3, 1)

        grid.addWidget(self._label("Y"), 3, 2)
        self.spin_y = QSpinBox()
        self.spin_y.setRange(0, 5000)
        self.spin_y.setValue(2500)
        self.spin_y.setSuffix(" px")
        self.spin_y.setToolTip("Initial Y")
        self.spin_y.setMinimumHeight(26)
        grid.addWidget(self.spin_y, 3, 3)

        self.btn_random_loc = QPushButton("Random Location")
        self.btn_random_loc.setMinimumHeight(26)
        self.btn_random_loc.setToolTip("Randomize initial location")
        self.btn_random_loc.setStyleSheet("background:#ffffff; border:1px solid #d1d5db; border-radius:4px; padding:4px 10px; font-weight:500;")
        grid.addWidget(self.btn_random_loc, 4, 0, 1, 4)

        # Row 5: Motion
        grid.addWidget(self._label("Motion"), 5, 0)
        self.combo_motion = QComboBox()
        self.combo_motion.addItems(MOTION_PROFILES_DISPLAY)
        self.combo_motion.setToolTip("Motion: applies to all beacons; Random gives each beacon random motion")
        self.combo_motion.setMinimumHeight(26)
        grid.addWidget(self.combo_motion, 5, 1, 1, 3)

        # Row 6: Speed
        grid.addWidget(self._label("Speed"), 6, 0)
        self.spin_speed = QSpinBox()
        self.spin_speed.setRange(5, 300)
        self.spin_speed.setValue(60)
        self.spin_speed.setSuffix(" px/s")
        self.spin_speed.setToolTip("Beacon speed — tuning parameter")
        self.spin_speed.setMinimumHeight(26)
        grid.addWidget(self.spin_speed, 6, 1)

        self.chk_random_speed = QCheckBox("Random")
        self.chk_random_speed.setToolTip("Randomize speed per beacon")
        grid.addWidget(self.chk_random_speed, 6, 2)

        grid.addWidget(self._label("Threshold"), 7, 0)
        self.spin_thresh = QSpinBox()
        self.spin_thresh.setRange(100, 255)
        self.spin_thresh.setValue(200)
        self.spin_thresh.setToolTip("Detector threshold — tuning parameter")
        self.spin_thresh.setMinimumHeight(26)
        grid.addWidget(self.spin_thresh, 7, 1)

        # Tuning hint
        grid.addWidget(self._label("Tuning: Speed & Threshold"), 7, 2, 1, 2)
        # Make hint small
        for i in range(grid.count()):
            item = grid.itemAt(i)
            w = item.widget() if item else None
            if w and isinstance(w, QLabel) and "Tuning: Speed" in w.text():
                w.setStyleSheet("color:#6b7280; font-size:9px; font-style:italic;")

        self.btn_randomize_motion = QPushButton("Randomize Motion")
        self.btn_randomize_motion.setMinimumHeight(28)
        self.btn_randomize_motion.setToolTip("Randomize motion profile for all beacons")
        self.btn_randomize_motion.setStyleSheet("background:#ffffff; border:1px solid #d1d5db; border-radius:4px; padding:4px 10px; font-weight:500;")
        grid.addWidget(self.btn_randomize_motion, 8, 0, 1, 2)

        self.btn_randomize_all = QPushButton("Randomize Parameters")
        self.btn_randomize_all.setMinimumHeight(28)
        self.btn_randomize_all.setToolTip("Randomize all parameters for multiple beacons including target")
        self.btn_randomize_all.setStyleSheet("background:#111827; color:#ffffff; border:1px solid #111827; border-radius:4px; padding:4px 10px; font-weight:600;")
        grid.addWidget(self.btn_randomize_all, 8, 2, 1, 2)

        # Status
        self.lbl_status = QLabel("1 beacon, Target #0")
        self.lbl_status.setStyleSheet("color:#6b7280; font-size:10px;")
        grid.addWidget(self.lbl_status, 9, 0, 1, 4)

        self.lbl_hint = QLabel("All beacons follow same rules; Random distributes per beacon.")
        self.lbl_hint.setWordWrap(True)
        self.lbl_hint.setStyleSheet("color:#6b7280; font-size:10px; font-style:italic; background:#ffffff; border:1px solid #e5e7eb; border-radius:4px; padding:4px;")
        grid.addWidget(self.lbl_hint, 10, 0, 1, 4)

        root.addWidget(box)
        root.addStretch()

        # Wiring
        self.spin_beacon_count.valueChanged.connect(self._on_beacon_count_changed)
        self.spin_target_index.valueChanged.connect(self._on_target_changed)
        self.combo_shape.currentTextChanged.connect(self._emit_multi_config)
        self.spin_size_w.valueChanged.connect(self._emit_multi_config)
        self.spin_size_h.valueChanged.connect(self._emit_multi_config)
        self.spin_x.valueChanged.connect(self._emit_multi_config)
        self.spin_y.valueChanged.connect(self._emit_multi_config)
        self.combo_motion.currentTextChanged.connect(self._emit_multi_config)
        self.spin_speed.valueChanged.connect(self._emit_multi_config)
        self.chk_random_speed.toggled.connect(self._emit_multi_config)
        self.chk_blinking.toggled.connect(self._emit_multi_config)
        self.spin_thresh.valueChanged.connect(lambda v: self.threshChanged.emit(int(v)))
        self.btn_random_loc.clicked.connect(self._randomize_location)
        self.btn_randomize_all.clicked.connect(self.randomizeAllRequested.emit)
        self.btn_randomize_motion.clicked.connect(self.randomizeMotionRequested.emit)

    def _label(self, text: str) -> QLabel:
        lbl = QLabel(text)
        lbl.setStyleSheet("color:#374151; font-size:11px;")
        return lbl

    def _on_beacon_count_changed(self, v: int) -> None:
        self.spin_target_index.setMaximum(max(0, int(v) - 1))
        if self.spin_target_index.value() >= int(v):
            self.spin_target_index.setValue(int(v) - 1)
        self._update_status()
        self._emit_multi_config()

    def _on_target_changed(self, idx: int) -> None:
        self._update_status()
        self.targetChanged.emit(int(idx))
        self._emit_multi_config()

    def _randomize_location(self):
        import random
        self.spin_x.blockSignals(True)
        self.spin_y.blockSignals(True)
        self.spin_x.setValue(random.randint(200, 4800))
        self.spin_y.setValue(random.randint(200, 4800))
        self.spin_x.blockSignals(False)
        self.spin_y.blockSignals(False)
        self._emit_multi_config()

    def _update_status(self) -> None:
        try:
            n = int(self.spin_beacon_count.value())
            tid = int(self.spin_target_index.value())
            shape = self.combo_shape.currentText()
            motion = self.combo_motion.currentText()
            self.lbl_status.setText(f"{n} beacons, Target #{tid}, {shape}, {motion}")
        except Exception:
            pass

    def collect_multi_config(self) -> MultiBeaconConfig:
        mapping = {
            "Straight Line": "linear",
            "Circular": "curved",
            "Figure 8": "figure_eight",
            "Spiral": "spiral",
            "Sin": "sinusoidal",
            "Zig-Zag": "zigzag",
            "Random": "random",
        }
        motion_display = self.combo_motion.currentText()
        profile = mapping.get(motion_display, "curved")
        shape = self.combo_shape.currentText().lower()
        return MultiBeaconConfig(
            beacon_count=int(self.spin_beacon_count.value()),
            target_index=int(self.spin_target_index.value()),
            shape=shape,
            size_w=int(self.spin_size_w.value()),
            size_h=int(self.spin_size_h.value()),
            x=float(self.spin_x.value()),
            y=float(self.spin_y.value()),
            profile=profile,
            speed=float(self.spin_speed.value()),
            blinking=bool(self.chk_blinking.isChecked()),
            speed_random=bool(self.chk_random_speed.isChecked()),
        ).validate()

    def set_config(self, cfg: MultiBeaconConfig, emit: bool = False) -> None:
        cfg = cfg.validate()
        self._config = cfg
        # Block signals
        for w in [self.spin_beacon_count, self.spin_target_index, self.spin_size_w, self.spin_size_h, self.spin_x, self.spin_y, self.spin_speed]:
            w.blockSignals(True)
        self.combo_shape.blockSignals(True)
        self.combo_motion.blockSignals(True)
        self.chk_random_speed.blockSignals(True)
        self.chk_blinking.blockSignals(True)
        try:
            self.spin_beacon_count.setValue(int(cfg.beacon_count))
            self.spin_target_index.setMaximum(max(0, int(cfg.beacon_count)-1))
            self.spin_target_index.setValue(int(cfg.target_index))
            # Shape
            shape_cap = str(cfg.shape).capitalize()
            idx = self.combo_shape.findText(shape_cap)
            if idx >= 0:
                self.combo_shape.setCurrentIndex(idx)
            self.spin_size_w.setValue(int(cfg.size_w))
            self.spin_size_h.setValue(int(cfg.size_h))
            self.spin_x.setValue(int(cfg.x))
            self.spin_y.setValue(int(cfg.y))
            # Motion reverse mapping
            rev = {
                "linear": "Straight Line",
                "curved": "Circular",
                "figure_eight": "Figure 8",
                "spiral": "Spiral",
                "sinusoidal": "Sin",
                "zigzag": "Zig-Zag",
                "random": "Random",
                "random_walk": "Random",
            }
            motion_disp = rev.get(str(cfg.profile).lower(), "Circular")
            idx = self.combo_motion.findText(motion_disp)
            if idx >= 0:
                self.combo_motion.setCurrentIndex(idx)
            self.spin_speed.setValue(int(cfg.speed))
            self.chk_random_speed.setChecked(bool(getattr(cfg, "speed_random", False)))
            self.chk_blinking.setChecked(bool(cfg.blinking))
        finally:
            for w in [self.spin_beacon_count, self.spin_target_index, self.spin_size_w, self.spin_size_h, self.spin_x, self.spin_y, self.spin_speed]:
                w.blockSignals(False)
            self.combo_shape.blockSignals(False)
            self.combo_motion.blockSignals(False)
            self.chk_random_speed.blockSignals(False)
            self.chk_blinking.blockSignals(False)
        self._update_status()
        if emit:
            self._emit_multi_config()

    def set_world_bounds(self, bounds: tuple[int, int]) -> None:
        self._world_bounds = bounds
        try:
            w, h = bounds
            self.spin_x.setRange(0, w)
            self.spin_y.setRange(0, h)
        except: pass

    def get_per_beacon_panels(self):
        return []

    def _emit_multi_config(self) -> None:
        try:
            cfg = self.collect_multi_config()
            self._config = cfg
            self._update_status()
            self.multiConfigChanged.emit(cfg)
        except Exception:
            pass
