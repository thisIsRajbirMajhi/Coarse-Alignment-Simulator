...
- **PID** `pid_controller.py:16` `kp1.5 ki0.1 kd0.25 tau0.02 deadband1px` `5°/s accel25 ptz.py:293` velocity mode.

## 4. AI — Tiny + Scorer + Predictor + Ranker (Plan §7 Pure OpenCV)
| Task | Model | Params | Train | Runtime |
|---|---|---|---|---|
| Detect | Tiny-CNN 32×32 `ai/verifier.py` 3×Conv16/32/64 GAP FC 4-class ~80k | 5k crops `headless+env DomainRand S&P/Gauss/stars4000/fog` PyTorch→ONNX 10ep CPU | `cv2.dnn` `p>0.6` `snr+=3dB` else Top8 `2ms/cand` |
| Acquire | MLP 7→1 `ai/scorer.py` 7→16→8→1 sigmoid ~200 | 2k assoc logs BCE | replaces hand `200·fov·beacon·temporal`, `>0.5 AND gate` |
| Track/Reacq | Temporal-MLP 32→64→2 `ai/predictor.py` ~4k `0.2ms` (fallback) or LSTM-64 35k `onnxruntime` | 2k traj CIRCULAR/FIGURE8/RANDOM | `x=KF+0.3Δ` `reacq seed need` |
| Search | MLP Ranker 6→20 `ai/tuner.py` `6→32→32→20 ~2k` | imitation 20-cell oracle | `max<0.7` fallback `SYSTEMATIC` |
| PID | Bayesian lookup `bg_std/haze→kp/kd` | 50 seeds BO | residual `±1°/s` |

Total `~86k <2MB` `99% cv2 ops` `≥28FPS`; LSTM optional `117k <5MB`. **Why not YOLO/RL:** YOLO 3M 15ms overkill 5-20px; Transformer/RL unstable vs PID ≤10px.

Training: `PyTorch CPU → torch.onnx.export → ai/models/*.onnx` `main.spec` bundles.
...
## 7. Pruning (Plan §1,6,9)
- Remote: 7→3 shapes, 7→5 profiles, OOK only, emission single switch. `motion LINEAR_RAMP`, `formation ARC/RECT/V`, `optics CW` deleted.
